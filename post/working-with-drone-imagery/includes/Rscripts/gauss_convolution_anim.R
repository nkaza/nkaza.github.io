...
gaussian_kernel <- matrix(c(
  ...
  2, 4, 2,
  1, 2, 1
), nrow = kernel_size, ncol = kernel_size, byrow = TRUE) / 16
...
# ---- Gaussian blur using terra::focal
r_out <- focal(r, w = gaussian_kernel, fun = sum, na.rm = TRUE)
mat_o <- as.matrix(r_out, wide = TRUE)
...
image_write(animation, "gaussian.gif")
